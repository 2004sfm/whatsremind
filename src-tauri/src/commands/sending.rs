...
pub async fn start_bulk_send(
    ...
    let mut clients = Vec::new();
    {
        ...
        for chunk in client_ids.chunks(100) {
            ...
            let sql = format!("SELECT id, phone_number, phone_number_2, name, code, debt FROM clients WHERE id IN ({})", placeholders);
    ...
    // Total = number of phone numbers to contact (1 or 2 per client)
    let total: u32 = clients.iter().map(|c| if c.phone_number_2.is_some() { 2 } else { 1 }).sum();
    if total == 0 {
        return Ok(());
    }
    ...
    spawn(async move {
        ...
        for client in clients {
            ...
            let _params = if template_name == "hello_world" || variables_count == 0 {
                vec![]
            } else {
                let mut all_params = vec![greeting.clone(), client.name.clone(), client.code.clone(), debt_str.clone()];
                all_params.truncate(variables_count as usize);
                all_params
            };

            // Build the list of phone numbers to send to for this client
            let mut phones: Vec<String> = vec![client.phone_number.clone()];
            if let Some(ref p2) = client.phone_number_2 {
                if !p2.trim().is_empty() {
                    phones.push(p2.clone());
                }
            }

            let mut client_had_success = false;
            ...
            for phone in phones {
                ...
                let send_params = if template_name == "hello_world" || variables_count == 0 {
                    vec![]
                } else {
                    // Re-spin the greeting for each number to vary the message
                    let g = parse_spintax(spintax_template);
                    let mut all_params = vec![g, client.name.clone(), client.code.clone(), debt_str.clone()];
                    all_params.truncate(variables_count as usize);
                    all_params
                };
                ...
                let result = if engine_clone == "unofficial" {
                    if let Some(port) = sidecar_port {
                        ...
                        for (i, p) in send_params.iter().enumerate() {
                    ...
                    meta_client.send_template_message(&phone, send_params).await
                };
            ...
            // Update last_sent on the client if at least one phone succeeded
            if client_had_success {
                let db = app_handle.state::<AppState>();
                let _ = db.db.lock().map(|conn| {
                    let _ = conn.execute(
